...
const pageList = computed(() => {
  const currentTypeData = copyIconList[currentActiveType.value];
  if (!currentTypeData || !Array.isArray(currentTypeData)) {
    return [];
  }
...
function setVal() {
  ...
  const colonIndex = inputValue.value.indexOf(":");
  if (colonIndex > -1) {
    // 图标格式为 "ep:add" 或 "ri:home" 等
    currentActiveType.value = inputValue.value.substring(0, colonIndex + 1);
    icon.value = inputValue.value.substring(colonIndex + 1);
  } else {
    // 如果没有冒号，默认使用ep:前缀
    currentActiveType.value = "ep:";
    icon.value = inputValue.value;
    // 更新完整的图标值
    inputValue.value = currentActiveType.value + icon.value;
  }
}
...
function onBeforeEnter() {
  ...
  const curIconIndex = currentTypeData.findIndex(i => i === icon.value);
  if (curIconIndex > -1) {
...
watch(
  ...
  () => {
    ...
    totalPage.value = currentTypeData.filter(i =>
...
<template>
  <div class="selector">
    <el-input
      v-model="inputValue"
      placeholder="请输入图标名称，如：ep:add 或 ri:home"
      clearable
    >
      <template #append>
        <el-popover
