<template>
  ...
  >
    ...
    <div v-if="type === 'avatar'" class="avatar-upload">
      ...
      >
        <div class="avatar-wrapper" :class="{ 'is-disabled': isUploading }">
    ...
    <div v-else-if="type === 'drag'" class="drag-upload">
      ...
      >
        ...
        <!-- 上传中的遮罩层 -->
        <div v-if="isUploading" class="drag-loading">
          <el-icon class="loading-icon">
            <Loading />
          </el-icon>
    ...
    <div v-else-if="type === 'card'" class="card-upload">
      <div class="upload-grid">
        ...
        <div
          ...
          @click="!isUploading && triggerUpload()"
        >
          <el-icon v-if="!isUploading" class="upload-icon">
            <Plus />
          </el-icon>
    ...
    <div v-else class="button-upload">
      ...
      <!-- 单文件模式下显示文件信息 -->
      <div v-if="limit === 1 && uploadedFiles[0]" class="single-file-info">
        <div class="file-item">
          <el-icon><Document /></el-icon>
          <span class="file-name">{{ uploadedFiles[0].name }}</span>
          <el-button type="danger" link size="small" @click="clearFiles">
            移除
          </el-button>
        </div>
        <div v-if="uploadedFiles[0].uploading" class="upload-progress">
          <el-progress :percentage="uploadedFiles[0].progress" />
        </div>
      </div>
    </div>
  </div>
...
const handleFileChange = async (file: UploadFile, files: UploadFiles) => {
  ...
  if (props.autoUpload) {
    ...
    if (props.limit === 1) {
      // 清理之前的本地预览URL
      if (
        uploadedFiles.value[0]?.url &&
        uploadedFiles.value[0].url.startsWith("blob:")
      ) {
        URL.revokeObjectURL(uploadedFiles.value[0].url);
      }
