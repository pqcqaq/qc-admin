...
import {
  getLoggingListWithPagination as getListApi,
  createLogging as createApi,
  updateLogging as updateApi,
  deleteLogging as deleteApi,
...
const currentRecord = ref<Logging>();

// 搜索表单数据
const searchParams = reactive<LoggingPageParams>({
  level: "",
  type: "",
  message: "",
  method: "",
...
// 查看数据
const viewData = ref<Logging>({} as Logging);

// 表格数据
const dataList = ref<Logging[]>([]);
...
const getTableData = async () => {
  loading.value = true;
  try {
    const params: LoggingPageParams = {
...
// 搜索处理
const handleSearch = (params: LoggingPageParams) => {
...
// 打开对话框
const openDialog = (type: "add" | "edit" = "add", row?: Logging) => {
...
// 删除日志记录
const handleDelete = async (row: Logging) => {
  try {
    loading.value = true;
